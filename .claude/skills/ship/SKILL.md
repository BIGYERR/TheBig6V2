...
1. `tests/gate.sh index.html <scratch>/base_V<N-1>.html`
2. Each new gate against the BASELINE too: `node tests/gates/g<N>_*.js <scratch>/base_V<N-1>.html` — expected to FAIL on the baseline (it proves the change). A gate green on both is vacuous; report it.
