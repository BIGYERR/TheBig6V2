...
0. **Did this session ship a version?** If `ia-version` did not move, it did not. Do NOT bump anything and do NOT invent a V-number: keep the working-file line where it is, add the session as its own bullet above the previous `Prior work` bullet, and give it a digest line named for what it was (`- **Post-V<N> … session — no build.**`). Elements 2–5 and 7 apply unchanged. Precedent: `Post-V154 rulings session — no build.`
...
6. **Digest** — add ONE line to the `## Recent sessions (digest)` block: `- **V<N>** — …`. **The block runs newest FIRST**, so the new line goes directly ABOVE the previous version's line, not at the end of the file. (Older entries further down are out of order — V182/V183 sit above V189 — so read the immediate neighbours, not the whole block, to place it.) A session that shipped no version still gets a line, named for what it was rather than a V-number: `- **Post-V<N> … session — no build.**`
7. **Session-start check** — `grep -oE 'content="[0-9]+"' index.html` must match the version you just folded, and there must be exactly ONE digest line for it. Count the digest line specifically, not the prefix: `grep -c '^- \*\*V<N> —'` (with the em-dash). A bare `grep -c '^- \*\*V<N>'` also catches §12 items such as `- **V190 watch items.**`, so it reads 3 or 4 on a healthy file — that is not a duplicate. If any earlier version has no digest line, say so in the entry and reconstruct it from the header, marked "(reconstructed)".
