...
function prehabClaim(tag, cand, base, handTable, tableApplies){
  ...
  if (base){
    ...
    else if (!belowBase.length) ok('B3 ' + tag + ': prehab is at or above the live V' + BASE_VER + ' baseline on ' + EQUIP.length + '/' + EQUIP.length + ' tiers (the RATCHET: a loss small enough to fit inside the V192 floor headroom still fails here)');
