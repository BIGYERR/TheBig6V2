// v209_gk_lattice.js — gatekeeper differential + identity fuzz for V209 (D140: the NSW long run tiered; F1/F2/carry item ban/power-core in the shared D18 pass).
  ...
  for(const k of ks){
    ...
    if(k !== 'top' && k !== 'CRASH'){
      ...
      if(kind !== 'cardio' && bd && cd){
        ...
        if(L){
          if(L.nrc && L.tier !== 'B'){
            if(dayCls[w+d] === undefined) dayCls[w+d] = canon(stripCarry(bd.sections)) === canon(cd.sections);
            if(dayCls[w+d]) cl = 'NRC tier ' + L.tier + ' carry-item removal';
          } else if(!L.nrc && L.tier === 'C'){
            if(dayCls[w+d] === undefined) dayCls[w+d] = canon(stripCarry(bd.sections)) === canon(cd.sections);
            if(dayCls[w+d]) cl = 'NSW tier C carry-item removal';
          } else cl = (L.nrc ? 'NRC' : 'NSW') + ' tier ' + L.tier + ' long-run day';
        }
      }
    }
