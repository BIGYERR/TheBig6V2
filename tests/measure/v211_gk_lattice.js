// v211_gk_lattice.js — gatekeeper differential + identity fuzz for V211 (D153 tier B pattern ban, D155 tier B deload pick).
...
// ── independent long-run-day oracle (from the D18/D33/D140 ruling text, not the engine) ──
function longOf(cardio){
  const c = cardio; if(!c || Array.isArray(c) || !c.dose) return null;
  let nrc = false;
  if(c.isNRC){ if(!/^long run/i.test(c.subtype||'') || /race day|time trial/i.test(c.subtype||'')) return null; nrc = true; if(/rehearsal/i.test(c.detail||'')) return {nrc, tier:'A', mins:null}; }
  else if(!(c.type === 'run' && c.dose.key === 'long')) return null;
  const mins = c.dose.k === 'time' ? (+c.dose.mins||0) : (+c.dose.mi||0) * (+c.dose.tgt||0) / 60;
  if(!mins) return null;
  return {nrc, tier: mins >= 75 ? 'A' : mins >= 45 ? 'B' : 'C', mins};
}
function stripCarry(secs){ const out = []; (secs||[]).forEach(s => { if(/carry/i.test(s.label||'') || /carry/i.test(s.coreHeader||'')) return; const its = s.items||[]; const kept = its.filter(it => !/carry/i.test((it&&it.name)||'')); if(its.length && !kept.length) return; out.push(kept.length === its.length ? s : {...s, items: kept}); }); return out; }
const setsOf = det => { let m = /^(\d+)\s*[x×]/.exec(det||''); if(m) return +m[1]; m = /\b(\d+)\s*sets?\b/i.exec(det||''); return m ? +m[1] : 1; };
const STRETCH = /stretch|mobility|90\/90|foam|worlds greatest/i;
const LEGHINGE = /deadlift|swing|squat|lunge|clean|snatch|jump|step-?up|hip thrust|good morning|\brdl\b|nordic|bound/i;
const HINGE_HAND = /deadlift|\brdl\b|romanian|good morning|hip thrust|glute bridge|back extension|pull-through|swing|hyperextension|reverse hyper|glute-ham|hip extension|\bghr\b|nordic/i;
function dayOracle(tag, p, side){
  R.orc = R.orc || {}; const O = R.orc[side] = R.orc[side] || {longDays:{}, carry:0, tierAlift:0, tierBpower:0, tierBsets:0, tierBleg:0, ex:[]};
  for(const w of Object.keys(p.weeks||{})) for(const d of Object.keys(p.weeks[w])){
    const x = p.weeks[w][d]; if(!x) continue; const L = longOf(x.cardio); if(!L) continue;
    const key = (L.nrc ? 'NRC' : 'NSW') + L.tier; O.longDays[key] = (O.longDays[key]||0) + 1;
    const secs = x.sections || [], items = [].concat(...secs.map(s => s.items||[]));
    const bad = [];
    if(items.some(it => /carry/i.test(it.name||''))){ O.carry++; bad.push('carry'); }
    if(L.tier === 'A' && secs.some(s => (s.items||[]).length && !/mobility|taper/i.test(s.label||'') )){ O.tierAlift++; bad.push('A lift'); }
    if(L.tier === 'B'){
      if(secs.some(s => /power|explosive/i.test((s.label||'') + ' ' + (s.coreHeader||'')) && (s.items||[]).length)){ O.tierBpower++; bad.push('B power'); }
      const n = items.filter(it => !STRETCH.test(it.name||'')).reduce((a, it) => a + setsOf(it.detail), 0); if(n > 8){ O.tierBsets++; bad.push('B sets ' + n); }
      if(items.some(it => LEGHINGE.test(it.name||'') && !STRETCH.test(it.name||''))){ O.tierBleg++; bad.push('B leg'); }
      if(items.some(it => HINGE_HAND.test(String(it.name||'')) && !STRETCH.test(it.name||''))){ O.tierBhinge = (O.tierBhinge||0) + 1; bad.push('B hinge ' + items.find(it => HINGE_HAND.test(String(it.name||''))).name); }
      const lifts = items.filter(it => it && it.name && !STRETCH.test(it.name)); if(!lifts.length && secs.length !== undefined && !x.rest){ const k2 = L.nrc ? 'zeroNRC' : 'zeroNSW'; O[k2] = (O[k2]||0) + 1; if(O.zeroEx === undefined) O.zeroEx = []; if(O.zeroEx.length < 4) O.zeroEx.push(tag + ' W' + w + d + ' ' + x.title + ' secs=' + secs.map(s => s.label||s.coreHeader).join('/')); }
    }
    if(bad.length && O.ex.length < 6) O.ex.push(tag + ' W' + w + d + ' ' + key + ' ' + bad.join(','));
  }
}
let _curB = null; const isRecov = w => { const wk = _curB && _curB.weeks[w]; return !!wk && Object.values(wk).some(x => x && /deload|recovery week/i.test(JSON.stringify(x.tags||'') + ' ' + (x.note||'') + ' ' + (x.weekNote||''))); };
function compare(tag, cfg, ctx){
  R.configs++;
  const b = build(B, cfg), b2 = build(B2, cfg), c = build(C, cfg);
  _curB = b.p; const fb = flat(b.p), fb2 = flat(b2.p);
  ...
  for(const k of ks){
    let cl = null;
    if(k !== 'top' && k !== 'CRASH'){
      const [w, d, kind] = k.split('|');
      const bd = b.p.weeks[w] && b.p.weeks[w][d], cd = c.p.weeks[w] && c.p.weeks[w][d];
      if(kind !== 'cardio' && bd && cd){
        const L = longOf(bd.cardio);
        if(L && L.tier === 'B') cl = (L.nrc ? 'NRC' : 'NSW') + ' tier B long-run day' + (isRecov(w) ? ' (deload week, D155)' : '');
      }
    }
    if(cl) bump(cl); else { R.unclN++; { const mm = tag.match(/^(V\d)/); const bk = mm[1] + ' ' + (k.split('|')[2]||k); R.unclBy = R.unclBy || {}; R.unclBy[bk] = (R.unclBy[bk]||0) + 1; } if(R.unclassified.length < 25) R.unclassified.push(tag + ' :: ' + k + ' :: base ' + String(fb.get(k)).slice(0,220) + ' || cand ' + String(fc.get(k)).slice(0,220)); }
  }
  dayOracle(tag, b.p, 'base'); dayOracle(tag, c.p, 'cand');
  return {b: b.p, c: c.p};
}
