// V220 rebase copy of v220_bare_rx.js: adds --age <bracket> (lattice was pinned 18-35) and a per-name bare tally in phase 2.
...
const P2I=process.argv.indexOf("--phase2"), P2=P2I>=0; const AGI=process.argv.indexOf("--age"), AGE=AGI>=0?process.argv[AGI+1]:"18-35"; console.log("AGE",AGE);
...
function mk(t,f,x,g,i,sd,travel){const isRace=!!g.id&&/half/.test(g.id);
  return {name:"M",primaryPath:g.id?(isRace?"event":"cardio"):"lift",cardioTypes:g.id?["run"]:[],
   ...
   eventTargeted:isRace,raceDate:isRace?"2026-12-06":null,liftingFocus:f,experience:x,ageBracket:AGE,equipment:t,unit:"lbs",
...
if(P2){
  ...
  const Q={builds:0,dataDiff:0,cards:0,ssCards:0,bare:0,bareSingle:0,bareRange:0,bareOutside:0,outsideEx:{},
    ...
    bareName:{},bareNameRule:{},swapCards:0,swapBare:0,swapRange:0,swapOutside:0,swapOK:0,swapBad:0,swapEx:null};
    ...
    Object.keys(pA.weeks).forEach(w=>Object.keys(pA.weeks[w]).forEach(d=>{const day=pA.weeks[w][d]; if(!day||day.rest||!Array.isArray(day.sections))return;
      ...
      day.sections.forEach(s=>{const ss=!!(s.superset||s.type==="superset");
        ...
        cA.forEach((c,k)=>{if(c.det==null)return; Q.cards++; const after=cB[k]&&cB[k].det;
          ...
          if(wasBare){inc(Q.bareName,c.nm); if(/^\d+(\s*[–-]\s*\d+)?$/.test(strip(c.det)))inc(Q.bareNameRule,c.nm);} if(!wasBare){Q.nonBareSS++; if(after!==c.det)Q.nonBareSSchanged++; const sh=shape(c.det); if(!Q.sample["nb:"+sh]&&Object.keys(Q.sample).filter(k=>k.startsWith("nb:")).length<6)Q.sample["nb:"+sh]=c.nm+" "+JSON.stringify(strip(c.det))+" -> "+JSON.stringify(strip(after));}
  ...
  const {sample,outsideEx,badEx,swapEx,bareName,bareNameRule,...rest}=Q; console.log("BARE BY NAME (ruling regex)",JSON.stringify(Object.entries(bareNameRule).sort((a,b)=>b[1]-a[1]))); console.log("PHASE2 counts",JSON.stringify(rest));
