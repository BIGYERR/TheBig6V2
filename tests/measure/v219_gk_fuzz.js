...
// ── FUZZ lattice: seeded PRNG, replaces U ──
U.length=0; { let r=0x2193; const rnd=()=>{r=(r*1103515245+12345)&0x7fffffff;return r/0x7fffffff;}; const pk=a=>a[Math.floor(rnd()*a.length)];
  const RP=[[],['sun'],['sat'],['wed'],['sun','wed'],['sat','sun'],['mon','thu'],['mon','wed','fri'],['tue','thu','sun'],['wed','sat'],['fri','sun']];
  const IJ=[null,null,null].concat(REG.flatMap(g=>ITIER.map(t=>({region:g,tier:t}))));
  const GL=['run_5k','run_10k','run_half','run_marathon','run_pace_goal','run_base','run_mile_time'];
  for(let i=0;i<600;i++){ const g=pk(GL), dated=rnd()<0.4; const c={name:'F',primaryPath:dated?'event':pk(['goal','fitness','lift']),eventTargeted:dated,raceDate:dated?pk(['2026-12-06','2027-01-17','2027-03-28','2026-11-15']):'',startDate:pk(['2026-09-21','2026-10-05',undefined]),
    cardioTypes:['run'],cardioGoals:{run:{id:g,label:g,mileBestMins:pk(['7','8','10','12']),mileBestSecs:pk(['0','15','30']),baselineDist:pk(['3','5','8']),baseline:'3mi',targetDist:'1.5',targetMins:'11',targetSecs:'0',paceUnit:'mi'}},
    liftingFocus:pk(FOC.concat(['support_athletic'])),experience:pk(EXPS),ageBracket:pk(AGES),equipment:pk(TIERS6),unit:'lbs',restDays:pk(RP).slice(),days:DAYS.slice(),bench:135,squat:155,deadlift:185,seed:Math.floor(rnd()*99999)+1};
    if(c.primaryPath==='lift'&&rnd()<0.5){c.cardioTypes=[];c.cardioGoals={};}
    if(rnd()<0.25){c.cardioTypes.push('bike');c.cardioGoals.bike={id:'bike_base',label:'Bb',baselineDist:'10',baseline:'10mi'};}
    if(rnd()<0.2){c.cardioTypes.push('swim');c.cardioGoals.swim={id:'swim_base',label:'Sb',baselineDist:'1000',baseline:'1000m'};}
    if(!c.startDate) delete c.startDate; const inj=pk(IJ); if(inj) c.injury=inj;
    U.push({lat:'F',eq:c.equipment,ik:inj?inj.region+'/'+inj.tier:'healthy',f:c.liftingFocus,mix:'fuzz',c}); } }
...
for(let ui=0; ui<U.length; ui++){ if(ui%NS!==SH) continue; const x=U[ui];
  ...
  if(crash) continue;
  if(true){ nIdent++; const b2=strip(IA.base.buildProgram(cl(x.c))); if(JSON.stringify(b2)!==JSON.stringify(P.base)){bump('IDENT base FAIL'); ex('IDENT',ui);} const c2=strip(IA.cand.buildProgram(cl(x.c))); if(JSON.stringify(c2)!==JSON.stringify(P.cand)) bump('IDENT cand FAIL'); }
  ...
  W.forEach(w=>ORD.forEach(d=>{ const b0=day('base',w,d), c0=day('cand',w,d); if(J(b0)===J(c0)) return; pch=true; bump('DIFF days');
    ...
    { const dn=d=>{const m={};names(d).forEach(n=>m[n]=(m[n]||0)+1);return Object.values(m).filter(v=>v>1).length;}; if(dn(c0)>dn(b0)){bump('DUP rose'); ex('DUPROSE','ui '+ui+' W'+w+' '+d);} }
...
fs.writeFileSync('/tmp/v219_gk/fz_'+SH+'.json',JSON.stringify({T,EX}));
